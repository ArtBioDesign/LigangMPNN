        ...
        for line in infile:
            if line.startswith(f">{name}, T=0.1"):
                ...
                if mutation_line.strip():
                    # data.append([seq_id, overall_confidence, mutation_line.strip(), parts[-1].split("=")[1].strip()  ])
                    data.append([overall_confidence, mutation_line.strip(), parts[-1].split("=")[-1].strip() ])

    return pd.DataFrame(data, columns=['Overall Confidence', 'Mutation Info', "chain"])  
...
def process_folder(folder_path, output_file, name):
    ...
    # 合并所有数据
    df = pd.concat(all_data, ignore_index=True) if all_data else pd.DataFrame()
    # 检查 'Mutation Info' 列是否存在
    if 'Mutation Info' in df.columns:
        # 处理存在 'Mutation Info' 列的情况
        df = (df[df['Mutation Info'] != ""]
                .sort_values('Overall Confidence', ascending=False)
                .drop_duplicates('Mutation Info')
                .sort_index())
        df.to_csv(output_file, index=False)
    else:
        # 如果没有 'Mutation Info' 列，打印提示信息或执行其他操作
        print("'Mutation Info' column not found in the DataFrame")
        # 可以根据需要执行其他逻辑，比如保存空文件
        df.to_csv(output_file, index=False)
...
def zip_directory(directory_path, output_zip):
   ...
   # print("打包目录",directory_path)
    #shutil.make_archive(output_zip, 'zip', root_dir=os.path.dirname(directory_path), base_dir=os.path.basename(directory_path))
...
def main():
    ...
    parser.add_argument('--site', default="", help='site file')
    ...
    if ".pdb" in site:
        site_csv_file = read_residues_from_pdb( args.site,  args.out_dir)
        run(site_csv_file, args.complex_pdb, args.out_dir, args.env, args.ligandpnn)
    else:
        run(args.site, args.complex_pdb, args.out_dir, args.env, args.ligandpnn)
